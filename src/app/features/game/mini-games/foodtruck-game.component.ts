...
export class FoodtruckGameComponent implements OnInit, OnDestroy {
  ...
  startAutoCook() {
    ...
    if (this.cookCount > 0) {
      ...
      this.autoCookInterval = setInterval(() => { if (this.isRunning) this.performAutoAction(); }, speed);
    }
  }
  stopAutoCook() { if (this.autoCookInterval) clearInterval(this.autoCookInterval); }
  performAutoAction() { 
    if (this.currentStack.length < this.currentOrder.length) { this.addIngredient(this.currentOrder[this.currentStack.length], undefined, true); } 
    else { this.validateBurger(); } 
  }
  ...
  resetGame(level: number, forceVip: boolean = false) {
    ...
    this.generateSmartOrder(); this.cdr.detectChanges();
  }
  ...
  startChrono() {
    ...
    const loop = () => {
      ...
      if (remaining <= 0) { this.displayTimeLeft = 0; this.spawnText('TEMPS ÉCOULÉ !', 200, 200, 'error'); this.failLevel(); } 
      else { this.displayTimeLeft = remaining; this.cdr.detectChanges(); this.rafId = requestAnimationFrame(loop); }
    };
  ...
  getButtonLabel() { return this.isFeverMode && !this.isVip ? '🔥 FEVER !' : (this.isVip ? '👑 SERVIR LE ROI' : '🛎️ ENVOYER'); }
  ...
  animateParticles() { if (this.particles.length === 0) return; this.particles.forEach(p => { p.x += p.speedX; p.y += p.speedY; p.speedY += 0.5; p.rotation += 5; }); this.particles = this.particles.filter(p => p.y < 600); if (this.particles.length > 0) requestAnimationFrame(() => { this.animateParticles(); this.cdr.detectChanges(); }); }
  spawnText(text: string, x: number, y: number, type: any) { const id = this.counter++; this.floatingTexts.push({ id, text, x: x || 200, y: y || 100, type }); setTimeout(() => { this.floatingTexts = this.floatingTexts.filter(ft => ft.id !== id); this.cdr.detectChanges(); }, 1000); }
