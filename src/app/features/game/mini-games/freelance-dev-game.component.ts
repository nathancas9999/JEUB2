...
export class FreelanceDevGameComponent implements OnInit, OnDestroy {
  ...
  readonly WORDS_EASY = ["chat", "chien", "ami", "pain", "eau", "mer", "ciel", "mur", "sac", "bus", "lit", "bol", "the", "sel", "coq", "or", "loi", "nez", "vis", "rue", "pin", "pot", "jeu", "gaz", "main", "pied", "joie", "papa", "cafe"];
  ...
  constructor(public gameStateService: GameStateService, private soundService: SoundService, private cdr: ChangeDetectorRef) {}
  ...
  ngOnInit() {
    this.gameStateService.gameState$.subscribe(state => {
        ...
        if (state.freelanceUpgrades) {
            ...
            const theme = IDE_THEMES.find(t => t.id === activeId);
            if (theme) {
  ...
  toggleStaff() { this.isStaffOpen = !this.isStaffOpen; this.isShopOpen = false; this.cdr.markForCheck(); }
  ...
  drinkCoffee() { if (this.isCoffeeActive || !this.coffeeAvailable) return; this.isCoffeeActive = true; this.coffeeAvailable = false; this.soundService.playPop(); setTimeout(() => { this.isCoffeeActive = false; this.cdr.markForCheck(); }, 5000); }
  ...
  showNotification(text: string) { this.notifications.push(text); setTimeout(() => { this.notifications.shift(); this.cdr.markForCheck(); }, 2000); }
  watchEmployee(emp: Employee) { this.watchedEmployee = emp; this.isStaffOpen = false; this.cdr.markForCheck(); }
  stopWatching() { this.watchedEmployee = null; this.isStaffOpen = true; this.cdr.markForCheck(); }
  ...
  startTimer() { 
      ...
      this.timerInterval = setInterval(() => { 
          ...
          this.cdr.markForCheck(); // Mise à jour UI à chaque tick
      }, 100); 
  }
  ...
  onInput(event: any) { 
      if (!this.activeMission || this.missionResult) return; 
      const val = event.target.value; 
      if (val.length > this.currentTargetLine.length) { 
          this.userInput = val.slice(0, this.currentTargetLine.length); 
          event.target.value = this.userInput; 
      } else { 
          this.userInput = val; 
      } 
      if (this.userInput.length > this.totalCharsTyped) { 
          const lastChar = this.userInput[this.userInput.length - 1]; 
          if (lastChar === ' ') this.soundService.playSpace(); else this.soundService.playRandomKey(); 
          this.checkInput(); 
      } 
  ...
  triggerError() { this.hasError = true; this.soundService.playError(); setTimeout(() => { this.hasError = false; this.cdr.markForCheck(); }, 300); }
  nextLine() { this.currentLineIndex++; if (this.activeMission && this.currentLineIndex >= this.activeMission.lines.length) { this.endMission(true); } else if (this.activeMission) { this.loadLine(this.activeMission.lines[this.currentLineIndex]); this.cdr.markForCheck(); } }
  ...
  endMission(success: boolean) { 
      ...
      if (success && this.activeMission) { 
          ...
          const comboBonus = this.maxComboReached; 
          this.lastReward = base + comboBonus; 
          this.gameStateService.addMoney(this.lastReward); 
          this.gameStateService.incrementContractsCompleted(); 
          if(this.activeMission.difficulty === 'BOSS') this.gameStateService.incrementBossBeaten(); 
          this.soundService.playSuccess(); 
      } else { 
  ...
  closeResult() { this.missionResult = null; this.activeMission = null; this.generateMissions(); this.cdr.markForCheck(); }
